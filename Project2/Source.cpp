...
int main() {
    ...
    Mat originalImage = imread("Testimg.jpg"); // Укажите путь к изображению машины
    ...
    vector<Vec2f> lines;
    HoughLines(edges, lines, 1, CV_PI / 180, 100); // Порог можно настроить
    ...
    HoughCircles(blurredImage, circles, HOUGH_GRADIENT, 1,
        blurredImage.rows / 16, 100, 30, 1, 100); // Параметры можно настроить
